...
- `switch_server`/`SwitchToEmbedded` tear down via `DisconnectForSwitch`, which guards `ReleaseDatasets` (its `nxSession1.CloseInactiveTables` is a server round-trip) and then calls `Disconnect`. `SwitchDatabaseTarget` uses `CloseDatabaseForSwitch`, which falls back to `ForceDisconnect` so that `OpenTargetDatabase` rebuilds the whole chain.
