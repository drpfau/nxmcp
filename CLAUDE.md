...
| `ExecuteWithReconnect` | catches `DBIERR_SERVERCOMMLOST` (`$2C0C`) and `DBIERR_REENTERED`, reconnects, retries the action **once**; on `DBIERR_NX_GENERALTIMEOUT` runs `RecoverAfterTimeout` and **re-raises** | wraps the actual round-trip; the only way to detect a stale-but-`Active` handle |
...
### Concurrency: one session, serialized tool execution
A NexusDB session is strictly **one request at a time**: a request arriving while the session's server-side lock (`TnxSimpleLock`, `nxsdServerEngine.pas`) is held by another thread is not queued but **rejected** — the server raises `DBIERR_REENTERED` ("System has been illegally re-entered"). The HTTP transport (`TIdHTTPServer`) runs every MCP request on its own Indy thread against the single shared `nxmodule`, so all tools and resources derive from `TSerializedToolBase<T>` / `TSerializedResourceBase<T>` (`nxmcp.SerializedAccess.pas`), which funnel `Execute`/`Read` through one process-wide critical section. **New tools must derive from these**, not from `TMCPToolBase<T>` directly: the library's `Execute` is not virtual, so the gate works by re-implementing `IMCPTool` on the derived class — a tool that derives from the MCPServer base compiles fine and silently bypasses the gate. (Under STDIO the transport is single-threaded and the gate is uncontended.)

`[Options] Timeout` (default 3000, `nxDatabase1.Timeout`) is the **per-request timeout**, sent with each request. The server enforces it *cooperatively* — the executing thread checks at cancellation checkpoints (`nxsrTimeoutHandling.nxCheckRemainingTime`, ~1s granularity) — so a scan in a checkpoint-poor stretch can outlive its timeout: the client already holds the timeout error while the server thread is still running **and still holds the session lock**. That is why serialization alone is not enough, and why the one observed reentrancy incident struck immediately after a query timeout. `ExecuteWithReconnect` therefore distinguishes three error classes:
- **comm-lost** (`DBIERR_SERVERCOMMLOST`): reconnect, retry once.
- **re-entered** (`DBIERR_REENTERED`): reconnect and retry once — the fresh session has a new ID and lock, so the retry cannot collide with the orphaned request still draining on the server.
- **timeout** (`DBIERR_NX_GENERALTIMEOUT`): `RecoverAfterTimeout`, then **re-raise** — the statement genuinely failed and retrying a slow query would only time out again. Recovery is `nxSession1.CancelProcessing` (documented cross-thread cancel: sets the failed-flag the server thread polls; bypasses the client-side session lock) followed by `Reconnect`. The reconnect matters twice over: it sidesteps the possibly-still-locked server session *and* discards the stale cancel flag which, had the request actually finished in the meantime, would fail the next operation on that session with "Processing was cancelled".

`batch_execute` intentionally stays outside `ExecuteWithReconnect` (its own rollback path owns error handling); it is covered by the gate, and a timeout mid-batch is recovered by the next tool call's reentered-handling.
